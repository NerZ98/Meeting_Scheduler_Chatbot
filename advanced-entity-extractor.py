...
class AdvancedEntityExtractor:
    def __init__(self, logger=None):
        ...
        try:
            ...
            # Load BART Keyphrase Extraction model
            self.keyphrase_extractor = pipeline(
                "text2text-generation", 
                model="aglazkova/bart_finetuned_keyphrase_extraction"
            )
            self.logger.info("BART Keyphrase Extraction model loaded successfully")
    ...
    def extract_keyphrases(self, text: str) -> List[str]:
        """
        Extract keyphrases using BART model
        
        Args:
            text (str): Input text to extract keyphrases from
        
        Returns:
            List[str]: Extracted keyphrases
        """
        try:
            # Use BART model to extract keyphrases
            keyphrases = self.keyphrase_extractor(
                text, 
                max_length=60, 
                num_return_sequences=3
            )
            
            # Extract and clean keyphrases
            extracted_phrases = [
                phrase['generated_text'].strip() 
                for phrase in keyphrases
            ]
            
            self.logger.debug(f"Extracted keyphrases: {extracted_phrases}")
            return extracted_phrases
        
        except Exception as e:
            self.logger.error(f"Error extracting keyphrases: {e}")
            return []
    ...
    def extract_entities(self, text: str) -> Dict[str, List[str]]:
        ...
        try:
            ...
            # Extract keyphrases
            entities['KEYPHRASES'] = self.extract_keyphrases(text)
            ...
            for match in duration_matches:
                ...
                if number:
                    ...
                    if re.search(r'(?:minute|min|mins|m)(?!\w)', text, re.IGNORECASE):
                        ...
                        # Default to minutes if no specific unit found
                        processed_durations.append(f"{number} mins")
